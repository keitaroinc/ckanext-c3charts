...
def c3charts_featured_charts(limit=3):
    ...
    context = {'model': model}
    for chart in charts:
        view = resource_view_show(context, {'id': chart['resource_view_id']})
        resource = resource_show(context, {'id': chart['resource_id']})
        dataset = package_show(context, {'id': chart['package_id']})
